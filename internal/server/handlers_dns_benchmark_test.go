...
func TestValidateDNSResponse(t *testing.T) {
	...
	if err := validateDNSResponse(query, 0xabcd); err == nil {
		t.Fatal("request accepted as response")
	}
	resp := make([]byte, len(query))
	copy(resp, query)
	resp[2] |= 0x80
	if err := validateDNSResponse(resp, 0xabcd); err != nil {
		t.Fatalf("valid response rejected: %v", err)
	}
	if err := validateDNSResponse(resp, 0x1111); err == nil {
		t.Fatal("mismatched id accepted")
	}
	if err := validateDNSResponse(resp[:8], 0xabcd); err == nil {
