...
describe("versioned login announcement", () => {
  it("separates session close from permanent dismissal", () => {
    expect(page).toContain('LOGIN_ANNOUNCEMENT_ID = "2026-09-v0.6.3-performance"');
  ...
  it("contains the requested release notes and responsive bubble layout", () => {
    expect(page).toContain("代理接口响应更轻");
    expect(page).toContain("大列表与概览更流畅");
    expect(page).toContain("搜索时同步过滤 provider 节点");
